...
class FindHarmonicImpulseResponse_Novak_code(object):
    """
    ...
    def GetHarmonicImpulseResponse(self):
        ...
        # print "sweep paramter %r" %L
        # print "harmonic order %r" %N
        # print "sampling rate %r" %fs
        # print "kernel length %r" %len_Hammer
        # print "len of impulse response %r" %len(h)

        # The time lags may be non-integer in samples, the non integer delay must be applied later
        dt_rem = dt - numpy.around(dt) # error in rounding the values # numpy.around - evenly round to given number of decimals
