...
def adaptive_initializedwith_sweep_bp():
    # virtial reference system
    print "Adaptive identification initialized with sweep identification, with log seperated bandpass filters"
...
def adaptive_initializedwith_sweep_weightedfilters():
    print "Adaptive identification initialized with sweep, with weighted filters"
    # virtial reference system
    filter_spec_tofind_noise = nlsp.log_weightingfilter(branches=branches,input=wgn_uniform_g.GetOutput())
    filter_spec_tofind_sweep = nlsp.log_weightingfilter(branches=branches,input=sine_g.GetOutput())
    ref_nlsystem = nlsp.HammersteinGroupModel_up(nonlinear_functions=nlsp.nl_branches(nlsp.function_factory.power_series,branches),
                                                 filter_irs=filter_spec_tofind_noise,
                                                 max_harmonics=range(1,branches+1))
    ref_nlsystem.SetInput(wgn_uniform_g.GetOutput())
...
adaptive_initializedwith_sweep_bp()
adaptive_initializedwith_sweep_weightedfilters()