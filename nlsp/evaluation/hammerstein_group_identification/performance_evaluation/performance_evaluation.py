...
def computationtime_evaluation(input_generator,branches,iden_method,Plot,rangevalue=10,save=False):
    ...
    for branches,signal_length in itertools.product(branch,length):
        sim = []
        iden = []
        reference = nlsp.WhiteGaussianGenerator(sampling_rate=input_generator.GetOutput().GetSamplingRate(),length=signal_length,
        ...
        for i in range(rangevalue):
            ...
            if save is True:
                found_filter_spec_save,nl_functions_save = nlsp.systemidentification("powerhgmweight",iden_method,branches,inputgenerator,ref_nlsystem.GetOutput())
