...
class AliasCompensatingHammersteinModelUpandDown(HammersteinModel):
    ...
    def __init__(self, input_signal=None, nonlin_func=nlsp.NonlinearFunction.power_series(degree=1),
        ...
        self._upsignal = sumpf.modules.ResampleSignal(samplingrate=self._asignal.GetOutput().GetSamplingRate() *
                                                                     nonlin_func.GetMaximumHarmonic(),
                                                        algorithm=self._resampling_algorithm)
        self._upfilter = sumpf.modules.ResampleSignal(samplingrate=self._asignal.GetOutput().GetSamplingRate() *
                                                                     nonlin_func.GetMaximumHarmonic(),
                                                        algorithm=self._resampling_algorithm)
    ...
    def _Connect(self):
        ...
        sumpf.connect(self._upsignal.GetOutput,self._nonlin_func.SetInput)
